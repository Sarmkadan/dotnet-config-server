...
public sealed class DiffViewerServiceTests
{
	...
	/// <summary>
	/// Logger used by the test class for structured logging.
	/// </summary>
	...
	public DiffViewerServiceTests()
	{
		...
		// Use the mock logger as a concrete ILogger for structured logging within the test class.
	...
	public async Task GetRollbackPreviewAsync_VersionNotFound_ThrowsConfigurationNotFoundException()
	{
		_logger.LogInformation(
			"Starting {TestMethod} with configurationId={ConfigurationId}, targetVersionId={TargetVersionId}",
			nameof(GetRollbackPreviewAsync_VersionNotFound_ThrowsConfigurationNotFoundException),
			Guid.NewGuid(),
			Guid.NewGuid());
		...
		try
		{
			...
			_logger.LogInformation(
				"{TestMethod} completed successfully – expected exception was thrown",
				nameof(GetRollbackPreviewAsync_VersionNotFound_ThrowsConfigurationNotFoundException));
		}
		catch (Exception ex)
		{
			_logger.LogError(ex,
				"Unexpected error in {TestMethod}",
				nameof(GetRollbackPreviewAsync_VersionNotFound_ThrowsConfigurationNotFoundException));
			throw;
		}
	}
	...
	public async Task GetRollbackPreviewAsync_NoActiveVersion_ReturnsPreviewWithNoCurrentVersion()
	{
		_logger.LogInformation(
			"Starting {TestMethod} with configurationId={ConfigurationId}, targetVersionId={TargetVersionId}",
			nameof(GetRollbackPreviewAsync_NoActiveVersion_ReturnsPreviewWithNoCurrentVersion),
			Guid.NewGuid(),
			Guid.NewGuid());
		...
		try
		{
			...
			_logger.LogInformation(
				"{TestMethod} completed successfully – preview returned with no active version",
				nameof(GetRollbackPreviewAsync_NoActiveVersion_ReturnsPreviewWithNoCurrentVersion));
		}
		catch (Exception ex)
		{
